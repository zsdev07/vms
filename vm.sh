...
EOF
    echo -e "${NC}"
    echo -e "${WHITE}Version 2.0${NC} | ${CYAN}High-Performance VM Management${NC}"
    echo
}
...
start_vm() {
    ...
    if load_vm_config "$vm_name"; then
        ...
        echo -e "${CYAN}╠══════════════════════════════════════╣${NC}"
        echo -e "${CYAN}║${NC}  Command: ${GREEN}ssh -p $SSH_PORT $USERNAME@localhost${NC}"
        ...
        # Build optimized QEMU command
        local qemu_cmd=(
            qemu-system-x86_64
            -enable-kvm
            -m "$MEMORY"
            -smp "cpus=$CPUS,cores=$CPUS,threads=1"
            -cpu host
        )

        # Quick boot optimizations
        if [[ "${QUICK_BOOT:-true}" == "true" ]]; then
            qemu_cmd+=(
                -machine type=q35,accel=kvm,usb=off,vmport=off,dump-guest-core=off
        ...
        print_status "ROCKET" "Launching QEMU with optimized settings..."
        echo
...
delete_vm() {
    ...
    read -p "$(print_status "INPUT" "Are you sure? Type 'yes' to confirm: ")" confirm
    if [[ "$confirm" == "yes" ]]; then
        if load_vm_config "$vm_name"; then
...
show_vm_info() {
    ...
    if load_vm_config "$vm_name"; then
        clear
        echo -e "${CYAN}╔═══════════════════════════════════════════════════════╗${NC}"
        echo -e "${CYAN}║${NC}          ${BOLD}VM INFORMATION: $vm_name${NC}                 ${CYAN}║${NC}"
        echo -e "${CYAN}╠═══════════════════════════════════════════════════════╣${NC}"
...
show_vm_performance() {
    ...
    if load_vm_config "$vm_name"; then
        clear
        echo -e "${CYAN}╔═══════════════════════════════════════════════════════╗${NC}"
        echo -e "${CYAN}║${NC}       ${BOLD}PERFORMANCE METRICS: $vm_name${NC}              ${CYAN}║${NC}"
        echo -e "${CYAN}╚═══════════════════════════════════════════════════════╝${NC}"
        echo
...
# ASCII art splash on first run
echo -e "${CYAN}"
cat << "EOF"
    ____            __  __       __               ___    ______  _____
   /  _/____  ___  / /_/ /_     / /____  ____ _  |__ \  / __ \ \/ / _ \
   / // __ \/ _ \/ __/ / /_____/ __/ _ \/ __ `/ __/ / / / / /\  / // /
 _/ // / / /  __/ /_/ / /_____/ /_/  __/ /_/ / / __/_/ /_/ / / / // /
/___/_/ /_/\___/\__/_/_/      \__/\___/\__,_/ /____(_)____/ /_/\___/
